...
from dataset.MulRan.utils import polar_to_cartesian, cartesian_to_polar
...
class BaseDataset(Dataset):
    def __init__(self, info_root, bev_dataset_root, w=900, h=200, res=4, measure_range=80.0, trans_threshold=None):
        ...
        assert os.path.exists(self.lidar_bev_root), print(f'LiDAR BEV root {self.lidar_bev_root} not exists!')
        self.radar_bev_root = os.path.join(self.bev_dataset_root, 'radar')
        assert os.path.exists(self.radar_bev_root), print(f'Radar BEV root {self.radar_bev_root} not exists!')
    ...
    def drop_polar_bev(self, l_bev, r_bev, rate_min=0.0, rate_max=0.1):
        l_bev_np_new = np.zeros_like(l_bev[0, :, :])
        l_bev_np = l_bev[0, :, :].numpy()
        valid_points = np.argwhere(l_bev_np > 0)
        ...
        for point in valid_points:
            h_ind, w_ind = point
            cur_value = l_bev_np[h_ind, w_ind]
            minus_value = 0
            for value_idx in range(int(cur_value)):
                if np.random.uniform(0, 1) < rate:
                    minus_value += 1
            l_bev_np_new[h_ind, w_ind] = l_bev_np[h_ind, w_ind] - minus_value
        l_bev_new = l_bev_np_new[np.newaxis, :, :].repeat(3, 0)
        l_bev_new = torch.from_numpy(l_bev_new)
        return l_bev_new, r_bev

    def annular_mask_polar_bev(self, l_bev, r_bev):
        l_range_start = math.floor(random.uniform(self.l_bev_h // 2, self.l_bev_h))
        # print(l_range_start, l_bev.shape)
        # l_bev[:, l_range_start:, :] = 0
        l_bev[:, 100:, :] = 0
        return l_bev, r_bev
...
class TripletDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augmentation:
            # query_l_bev, query_r_bev = self.jitter_polar_bev(query_l_bev, query_r_bev)
            # query_l_bev, query_r_bev = self.drop_polar_bev(query_l_bev, query_r_bev)
            query_l_bev, query_r_bev = self.translate_polar_bev(query_l_bev, query_r_bev)
            # query_l_bev = self.erasing(query_l_bev)
        ...
        if self.is_batch_hard_mining:
            ...
            neg_sample = self.latent_vectors[np.array(self.neg_index[query_index]), :]
            ...
            neg_idx = np.array(self.neg_index[query_index])[neg_idx_in_sample]
        else:
    ...
    def update_latent_vectors(self, vecs_filepath):
        ...
        latent_vectors_full = np.zeros([len(self.database_index_n_pos), 512])
...
class QueryDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #     if 'fog_0.0163' not in self.lidar_bev_root:
        #         self.lidar_bev_root = self.lidar_bev_root + '_fog_0.0163'
