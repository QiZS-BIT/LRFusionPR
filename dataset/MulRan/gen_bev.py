...
def gen_bev(bev_fileroot, info_filepath, bs_whole):
    ...
    for i in tqdm(range(0, bs_whole.shape[0])):
        ...
        filename_r = os.path.basename(cur_info['lidar_infos']['LIDAR_TOP']['filename']).split('.')[0] + '.png'
        cv2.imwrite(os.path.join(radar_bev_root, filename_r), r_bev_img)
...
if __name__ == '__main__':
    ...
    db_ind_root = os.path.join(dataset_root, 'sejong_db.npy')
    train_query_ind_root = os.path.join(dataset_root, 'sejong_train_query.npy')
    test_db_ind_root = os.path.join(dataset_root, 'sejong_test_db.npy')
    test_query_ind_root = os.path.join(dataset_root, 'sejong_test_query.npy')
    ind_db = np.load(db_ind_root)
    ind_train_query = np.load(train_query_ind_root)
    ind_tdb_query = np.load(test_db_ind_root)
    ind_test_query = np.load(test_query_ind_root)
    ind_whole = np.concatenate([ind_db, ind_train_query, ind_tdb_query, ind_test_query], axis=0)

    gen_bev(bev_root, infos_path, ind_whole)

    # -------------------------Riverside-----------------------------
    bev_root = os.path.join(dataset_root, 'riverside_multi_bev')
    infos_path = os.path.join(dataset_root, 'mulran_infos-riverside.pkl')
    db_ind_root = os.path.join(dataset_root, 'riverside_test_db.npy')
    query_ind_root = os.path.join(dataset_root, 'riverside_test_query.npy')
    ind_db = np.load(db_ind_root)
    ind_query = np.load(query_ind_root)
    ind_whole = np.concatenate([ind_db, ind_query], axis=0)

    gen_bev(bev_root, infos_path, ind_whole)

    # ----------------------------DCC--------------------------------
    bev_root = os.path.join(dataset_root, 'dcc_multi_bev')
    infos_path = os.path.join(dataset_root, 'mulran_infos-dcc.pkl')
    db_ind_root = os.path.join(dataset_root, 'dcc_test_db.npy')
    query_ind_root = os.path.join(dataset_root, 'dcc_test_query.npy')
    ind_db = np.load(db_ind_root)
    ind_query = np.load(query_ind_root)
    ind_whole = np.concatenate([ind_db, ind_query], axis=0)
